...
void ZCommand::loadConfig()
{
  ...
  // Fallback to NVS settings if config is stale or defaults to 1200
  uint32_t nvsBaud = mode_get_baud_rate();
  ...
  uint32_t nvsSerialConfig = mode_get_serial_config();
  if((serialConfig == DEFAULT_SERIAL_CONFIG) || (serialConfig <= 0))
    serialConfig = (SerialConfig)nvsSerialConfig;
...
ZResult ZCommand::doBaudCommand(int vval, uint8_t *vbuf, int vlen)
{
  ...
  if(baudChk != baudRate)
  {
    if((baudChk<128)||(baudChk>921600))
      return ZERROR;
    baudRate = baudChk;
    mode_set_baud_rate(baudRate); // Sync back to NVS
    changeBaudRate(baudRate);
  }
  if(serialConfig != configChk)
  {
    serialConfig = (SerialConfig)configChk;
    mode_set_serial_config((uint32_t)serialConfig); // Sync back to NVS
    changeSerialConfig(serialConfig);
  }
  return ZOK;
}
