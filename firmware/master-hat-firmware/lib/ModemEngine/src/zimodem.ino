...
// Copico: stubs replacing the old New_Porta_Serial UI/mode managers
...
#define DEFAULT_SERIAL_CONFIG SERIAL_8N2
...
#  define DEFAULT_PIN_TXD 2   // CoCo TX -> SP3232 -> CoCo RX
#  define DEFAULT_PIN_RXD 3   // CoCo RX <- SP3232 <- CoCo TX
// All modem control signal pins disabled (-1) on the Super Mini.
// GPIO4 is our ENTER button, GPIO9 is our SELECT button.
// There are no free GPIO pins for flow control on this board.
#  define DEFAULT_PIN_DCD -1
#  define DEFAULT_PIN_CTS -1
#  define DEFAULT_PIN_RTS -1
...
#  undef  INCLUDE_SD_SHELL
// Disable flow control -- use software (XON/XOFF) or NONE
#  undef  DEFAULT_FCT
#  define DEFAULT_FCT FCT_DISABLED
...
# endif
# define debugPrintf ui_set_modem_status
...
static bool connectWifi(const char* ssid, const char* password, IPAddress *ip, IPAddress *dns, IPAddress *gateWay, IPAddress *subNet)
{
  ...
  if(WiFi.status() == WL_CONNECTED) {
    ...
    // Inject dynamic connection status to the OLED screen
    String uiStr = "Connected: " + String(ssid) + "\n" + WiFi.localIP().toString();
    ui_set_status(uiStr.c_str());
  }
...
// Renamed from setup() to allow FreeRTOS scheduling
void zimodem_setup() 
{
...
# ifdef DEFAULT_PIN_RXD
    uint32_t activeBaud = mode_get_baud_rate();
    debugPrintf("Using BPS %d, RXD %d, TXD %d\r\n",activeBaud,DEFAULT_PIN_RXD, DEFAULT_PIN_TXD);
    HWSerial.begin(activeBaud, SERIAL_8N2, DEFAULT_PIN_RXD, DEFAULT_PIN_TXD);
# else
    uint32_t activeBaud = mode_get_baud_rate();
    HWSerial.begin(activeBaud, SERIAL_8N2);  //Start Serial
# endif
    ...
    if(!digitalRead(PIN_FACTORY_RESET))
    {
      if(resetPushTimer != 1)
      {
        ...
        if((millis() - resetPushTimer) > 5000)
        {
          ...
          if (SPIFFS.exists(CONFIG_FILE_OLD)) SPIFFS.remove(CONFIG_FILE_OLD);
          if (SPIFFS.exists("/zphonebook.txt")) SPIFFS.remove("/zphonebook.txt");
          if (SPIFFS.exists("/zlisteners.txt")) SPIFFS.remove("/zlisteners.txt");
          if (SPIFFS.exists("/znick.txt"))      SPIFFS.remove("/znick.txt");
...
// Renamed from loop() to allow FreeRTOS scheduling
// RTC sniffer state for WiModem mode
static const char rtc_ztrigger[] = "+++RTC?\r";
static uint8_t rtc_zmatch = 0;
...
void zimodem_loop() 
{
  ...
  {
    // --- RTC SNIFFER: intercept +++RTC? before Zimodem sees the bytes ---
    uint8_t peekByte = HWSerial.peek();
  if(HWSerial.available())
  {
